...
static int attach_uprobe(struct memleak_bpf *skel)
{
...
static void walk_hash_elements(struct bpf_map *map, struct bpf_map *m2)
{
	//const size_t stack_traces_key_size = bpf_map__key_size(m2);
		...
		fprintf(stderr, "Leak address: %p, size: %lu; id: %d\n", (void*)info.address, info.size, info.stack_id);
...
static void parse_input(int argc, char **argv)
{
	...
	fprintf(stderr, "Start memleak check on: %d with interval: %d\n", attach_pid, interval);
}
